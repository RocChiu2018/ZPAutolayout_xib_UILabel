...
/**
 ...
 在约束UILabel控件的时候为了使当此控件里面的文字很少的时候内容也能够充满此控件，所以不能把此控件的宽度设为固定值，要规定宽度的一个最大值，即宽度要小于等于这个值。在xib中约束完控件的宽度以后，要对控件的宽度约束进行修改，把宽度约束的Relation选项设置为"Less Than or Equal"（小于等于）就可以了。
 */
